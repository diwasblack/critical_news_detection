...
The vectors for documents are then divided into a training set and a test set where the training set is used to train and validate the models and the test set is used to evaluate the performance of the models. The steps that were used to train and test the classifiers is shown in \figureref{fig:training}.
...
The trained model could then be used to classify a news article. In order to predict a class label for a news article, the text would have to be converted to a vector using the TF-IDF algorithm. The vector could then be used to predict the class label. The process of predicting class labels using a pre-trained classifier is summarized in \figureref{fig:prediction}.
...
\subsection{Training}
At first, the dataset is randomly separated into training and test sets using a 80-20 split. The test set remains unused during the training phase and training proceeds with the training dataset. The hyperparameters for the models are optimized by searching randomly through the hyperparameter space and evaluating the performance using a 5-fold cross validation technique.
